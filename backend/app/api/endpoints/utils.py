# File: backend/app/api/endpoints/utils.py
...
def check_book_quantity(book_id: str, db: Session = Depends(get_db)):
    """
    Sử dụng DB Function để kiểm tra số lượng hiện có của một cuốn sách.
    """
...
def get_unreturned_borrows(db: Session = Depends(get_db)):
    """
    Sử dụng Stored Procedure để lấy danh sách tất cả các phiếu mượn có trạng thái "Đang mượn".
    """
